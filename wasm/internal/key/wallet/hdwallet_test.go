...
func TestNewCommitteeKeyFromchameleonPrivateKey(t *testing.T) {
	tests := []string{
	...
	for _, tt := range tests {
		...
		incKey, _ := chameleonkey.NewCommitteeKeyFromSeed(seed, pubKey)
		fmt.Println(incKey.ToBase58())
		fmt.Println(incKey.GetMiningKeyBase58(common.BlsConsensus))
	}

	x := chameleonkey.NewCommitteePublicKey()
