...
func TestCoinV2BytesAndSetBytes(t *testing.T) {
	for i := 0; i < 5; i += 1 {
		...
		keyset := new(chameleonkey.KeySet)
...
func testCoinV2ConcealedTampered(c *CoinV2, ks *chameleonkey.KeySet, t *testing.T){
	saved := c.GetAmount()
...
//	coin.SetInfo([]byte("chameleon chain"))
//
//	err = coin.CommitAll()
//	assert.Equal(t, nil, err)
//
...
func TestCoinV1CommitAll(t *testing.T) {
	for i := 0; i < 3; i++ {
		...
		coin.CommitAll()
		coin.info = []byte("chameleon chain")
...
func TestOutputCoinBytesSetBytes(t *testing.T) {
	...
	coin.CoinDetails.info = []byte("chameleon chain")
