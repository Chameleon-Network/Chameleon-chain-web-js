  ...
  try {
    txsTransactor.forEach((txt) => {
      const foundTxpIndex = _txsPToken.findIndex(
        (txp) =>
          txp.chameleonTx === txt.txId ||
          txp.chameleonTxToPayOutsideChainFee === txt.txId
      );
      if (foundTxpIndex > -1) {
        const { amount, fee, txId } = txt;
        const foundTxp = _txsPToken[foundTxpIndex];
        const { chameleonAmount, inchainFee, decentralized } = foundTxp;
        if (!chameleonAmount) {
          _txsPToken[foundTxpIndex].chameleonAmount = amount;
        }
        if (!inchainFee) {
          switch (decentralized) {
            case 0: {
