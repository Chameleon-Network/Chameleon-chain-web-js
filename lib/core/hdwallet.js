...
import { byteToHexString, getShardIDFromLastByte } from "../common/common";
import {
  checkEncode,
  checkSumFirst4Bytes,
  checkDecode,
...
import { base64Encode, hashSha3BytesToBytes } from "../privacy/utils";
...
async function NewKey(seed, index = 0, depth = -1) {
  ...
  const chameleonKeySet = new KeySet();
  await chameleonKeySet.generateKey(childHdKey.privateKey);
  const chameleonChildKey = new KeyWallet();
  chameleonChildKey.ChildNumber = new bn(index).toArray("be", ChildNumberSize);
  chameleonChildKey.ChainCode = childHdKey.chainCode;
  chameleonChildKey.Depth = depth + 1;
  chameleonChildKey.KeySet = chameleonKeySet;

  return chameleonChildKey;
}
